...
export default class MailchimpList extends SyncAgent {
  ...
  constructor(ship, hull, req, MailchimpClientClass) {
    super(ship, hull, req);
    this.listId = this.ship.private_settings.mailchimp_list_id;
  ...
  deleteAudience(audienceId, segmentId) {
    ...
    this._audiences = null;
    return this.mailchimpClientRequest
      .delete(`/lists/${this.listId}/segments/${audienceId}`)
      .then(() => {
   ...
   * @return {Promise}
   */
  removeUsersFromAudiences(users = [], audienceId = null) {
    // const usersToRemove = users.filter(
    //   u => !_.isEmpty(u.email) && !_.isEmpty(u["traits_mailchimp/unique_email_id"])
    // );
      ...
      const batch = usersToRemove.reduce((ops, user) => {
        ...
        if (hash) {
          _.map(audiences, ({ audience }) => {
            ops.push({
              method: "delete",
              path: `/lists/${this.listId}/segments/${audience.id}/members/${hash}`
            });
          });
        }
        return ops;
   ...
   */
  removeAudiences() {
    this.hull.logger.info("removeAudiences", this.fetchAudiences());
    return this.fetchAudiences()
    .then(segments => {
      return segments.map(segment => {
        this.hull.logger.info("removeAudience", segment.id);
        return this.mailchimpClientRequest
          .delete(`/lists/${this.listId}/segments/${segment.id}`);
      });
    })
  ...
  fetchAudiences() {
    return this.mailchimpClientRequest
      .get(`/lists/${this.listId}/segments`)
      .query({ type: "static", count: 250 })
      .then(
        (response) => response.body.segments,
   ...
   */
  // getEventsAgent() {
  //   const { apiKey } = this.mailchimpClientRequest;
  //   // const client = this.getClient();
  //   if (!eventsAgents[apiKey]) {
  //     eventsAgents[apiKey] = new EventsAgent(this.mailchimpClientRequest, this.hull, this.getCredentials());
  //   }
  //   return eventsAgents[apiKey];
  // }
